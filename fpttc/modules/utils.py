...
def feature_add_position_single(feature0, attn_splits, feature_channels):
    pos_enc = PositionEmbeddingSine(num_pos_feats=feature_channels // 2)
...
def upsample_scale_with_mask(scale, up_mask, upsample_factor,
                            is_depth=False):
    # convex upsampling following raft

    mask = up_mask
    b, scale_channel, h, w = scale.shape
    mask = mask.view(b, 1, 9, upsample_factor, upsample_factor, h, w)  # [B, 1, 9, K, K, H, W]
    mask = torch.softmax(mask, dim=2)

    multiplier = 1
    up_scale = F.unfold(multiplier * scale, [3, 3], padding=1)
    up_scale = up_scale.view(b, scale_channel, 9, 1, 1, h, w)  # [B, 1, 9, 1, 1, H, W]

    up_scale = torch.sum(mask * up_scale, dim=2)  # [B, 1, K, K, H, W]
    up_scale = up_scale.permute(0, 1, 4, 2, 5, 3)  # [B, 1, K, H, K, W]
    up_scale = up_scale.reshape(b, scale_channel, upsample_factor * h,
                              upsample_factor * w)  # [B, 1, K*H, K*W]
