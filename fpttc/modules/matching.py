...
def global_correlation_softmaxx(feature0, feature1,
...
def local_correlation_softmaxx(feature0, feature1, local_radius,
                              padding_mode='zeros',
                              ):
...
def scale_correlation_softmax(feature0, feature1):
    # global correlation
    b, c, h, w = feature0.shape
    _, _, h_, w_ = feature1.shape
    #print(feature0.shape, feature1.shape)
    feature0 = feature0.view(b, c, -1).permute(0, 2, 1)  # [B, H*W, C]
    feature1 = feature1.view(b, c, -1)  # [B, C, H*W]

    correlation = torch.matmul(feature0, feature1).view(b, h, w, h_, w_) / (c ** 0.5)  # [B, H, W, H, W]

    # flow from softmax
    init_grid = torch.ones((b, h_, w_)).to(correlation.device)  # [B, 1, H, W]
    grid = init_grid.view(b, 1, -1).permute(0, 2, 1)  # [B, H*W, 1]

    correlation = correlation.view(b, h * w, h_ * w_)  # [B, H*W, H*W]

    prob = F.softmax(correlation, dim=-1)  # [B, H*W, H*W]

    #print(prob.shape, grid.shape)
    correspondence = torch.matmul(prob, grid).view(b, h, w, 1).permute(0, 3, 1, 2)  # [B, 1, H, W]

    return correspondence # [B,H,W,1]
...
def local_scale_correlation(feature0, feature1, local_radius=3,
                              padding_mode='zeros',
                              ):
    b, c, h, w = feature0.size()
    coords_init = coords_grid(b, h, w).to(feature0.device)  # [B, 2, H, W]
    ...
    prob = F.softmax(corr, -1).view(b, h, w, -1).permute(0,3,1,2).contiguous()  # [B, H*W, (2R+1)^2]
