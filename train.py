...
def main():
    ...
    optimizer = torch.optim.AdamW([{"params": model.parameters()}], lr=max_lr, weight_decay=args.weight_decay)
    ...
    model_path = "models/kitti.pth.tar" if args.resume is None else args.resume

    trainer = TTCTrainer(model=model, dataset=dataset, optimizer=optimizer, args=args, 
                        start_epoch=epoch, device=device, model_path=model_path, 
